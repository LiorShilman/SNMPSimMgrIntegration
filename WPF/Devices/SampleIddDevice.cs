...
namespace SNMPSimMgr.Devices
{
    ...
    public static class SampleIddDevice
    {
        ...
        /// </summary>
        public static List<IddFieldDef> BuildFields() => new List<IddFieldDef>
        {
            // ══════════════════════════════════════════════════════════════
            ...
            new IddFieldDef
            {
                ...
                IsWritable  = false,
                DefaultValue = "1",
                Options     = new Dictionary<string, int> { ["Fault"] = 0, ["OK"] = 1 }
            },
            ...
            new IddFieldDef
            {
                ...
                IsWritable  = true,
                DefaultValue = "0",
                Options     = new Dictionary<string, int> { ["Disabled"] = 0, ["Enabled"] = 1 }
            },
        ...
        public static void HandleSet(string fieldId, string value)
        {
            ...
            UpdateFields(new Dictionary<string, string>
            {
                ["idd.info.temperature"] = "73",
                ["idd.info.uptime"] = "172800",
                ["idd.info.hwStatus"] = "1",
            });
        }

        // ══════════════════════════════════════════════════════════════════
        // Hardware → Angular: push field updates FROM your system TO the client
        // ══════════════════════════════════════════════════════════════════

        /// <summary>
        /// Call this when your hardware sends a status update for a SINGLE field.
        /// The field ID must match one of the IDD fields defined in BuildFields().
        /// Angular will update the panel value in real-time.
        ///
        /// Example — hardware callback reports new temperature:
        ///   SampleIddDevice.UpdateField("idd.info.temperature", "73");
        /// </summary>
        public static void UpdateField(string fieldId, string value)
        {
            System.Diagnostics.Debug.WriteLine(
                $"[{DeviceName}] Hardware → Angular: {fieldId} = {value}");

            Startup.IntegrationWiring.NotifyIddSet(DeviceId, fieldId, value);
        }

        /// <summary>
        /// Call this when your hardware sends multiple field updates at once.
        /// More efficient than calling UpdateField() in a loop — sends one batch broadcast.
        ///
        /// Example — periodic hardware poll returns multiple values:
        ...
        ///   });
        /// </summary>
        public static void UpdateFields(Dictionary<string, string> fieldValues)
        {
            System.Diagnostics.Debug.WriteLine(
                $"[{DeviceName}] Hardware → Angular: batch update {fieldValues.Count} fields");

            Startup.IntegrationWiring.NotifyIddBatch(DeviceId, fieldValues);
        }
    }
}
